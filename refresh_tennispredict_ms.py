# refresh_tennispredict_ms.py
# requests-driven + retry + progress + MMDD input/argv
#
# Key behavior (MS):
# - Fetch ALL records from API
# - Group by date_short (e.g. "03/04") and write ms_player_count_MMDD_*.csv for EVERY date in dataset
# - Publish ALL generated ms_player_count_*.csv to docs/data and update ms_manifest.json/ms_latest.json
#
# Run:
#   winpty python -u refresh_tennispredict_ms.py
#   START_MMDD=0304 winpty python -u refresh_tennispredict_ms.py
#   MMDD=0304 winpty python -u refresh_tennispredict_ms.py   (forces day view file only)
...
# Data API event id (from XHR: /zh/survivor/event/136/2026/detail)
API_EVENT_ID = 136
...
def ds_to_tag(ds: str) -> str:
    return (ds or "").replace("/", "")  # "03/04" -> "0304"
...
def main():
    ...
    for ds in sorted(groups.keys()):
        ...
        f_main = f"ms_player_count_{tag2}_main.csv"
        f_alt = f"ms_player_count_{tag2}_alt.csv"
    ...
    # Copy ALL generated ms_player_count files into docs/data (overwrite)
    published = 0
    ...
    # Build ms_manifest.json from docs/data MS main files
    dates = []
    ...
    print(f"[publish] copied {published} ms_player_count files into {data_dir}", flush=True)
    print(f"[publish] updated docs/data/ms_manifest.json and ms_latest.json (latest={latest}, version={version})", flush=True)
